...
class Model(BaseModel):
    def __init__(self, config:dict):
        ...
        self.sent_encoder = DynamicRNN(config["word_proj_hidden_size"], config["sentence_hidden_size"], config["sentence_layers"],
                                        dropout=config["dropout"], bidirectional=True, rnn_type="GRU")
                                        
        self.sent_size = 2 * config["sentence_layers"] * config["sentence_hidden_size"]
        self.visual_attention_size = config["visual_attention_size"]
        self.sent_proj_fc = nn.Linear(self.sent_size, self.visual_attention_size)
        self.sent_proj_tanh = nn.Tanh()
        self.img_proj_fc = nn.Linear(config["img_input_size"], self.visual_attention_size)
        ...
        self.v = nn.Parameter(data=torch.from_numpy(np.random.uniform(-config["uniform_bound"], config["uniform_bound"],
                              size=(self.visual_attention_size))).type(torch.float32))
        self.visual_softmax = nn.Softmax(dim=2)
        
        self.doc_proj_fc = nn.Linear(self.visual_attention_size, self.visual_attention_size)
        ...
        self.output_layer = OutputLayer(config["task"], self.visual_attention_size, config["output_size"], dropout=config["dropout"])
