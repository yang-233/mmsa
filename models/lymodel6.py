from models.multiheadattention import MultiHeadAttentionScore
import torch
...
class Model(BaseModel):
    def __init__(self, config:dict) -> None:
        ...
        self.bbox_score = MultiHeadAttentionScore(self.word_output_size, config["fusion_nheads"], config["uniform_bound"])
    ...
    def forward(self, X):
        ...
        attn_bbox, _score = self.attn(output.permute(1, 0, 2), transform_x, 
        ...
        bbox = forget_val * attn_bbox


        # fusion = self.tanh2(self.forget_gete_text_2(output) + self.forget_gate_img_2(forget_val))
        # fusion_output = self.fusion_encoder(fusion_input, text_key_padding_mask)
        score = self.bbox_score(bbox) # (batch_size, seq)
        h = score @ output
        h = h.squeeze()
