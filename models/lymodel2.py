...
class Model(BaseModel):
    def __init__(self, config:dict) -> None:
        ...
        # _layer = nn.TransformerEncoderLayer(config["img_input_size"], config["bbox_nhead"])
        # self.img_transformer = nn.TransformerEncoder(_layer, config["img_encoder_layers"])
        ...
        self.fusion_encoder = DynamicRNN(self.word_output_size * 2, hidden_size=config["fusion_hidden_size"], 
    ...
    def forward(self, X):
        ...
        attn_output, score = self.attn(output.permute(1, 0, 2), transform_x, 
                        transform_x, key_padding_mask=img_key_padding_mask)
        attn_output = attn_output.permute(1, 0, 2) # (batch_size, seq, d)
        fusion_input = torch.cat((output, attn_output), dim=2)

        output, (hn, _) = self.fusion_encoder(fusion_input, lens)
